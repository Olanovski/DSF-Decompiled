feedbackSystem.registerHUD("Tutorial mission 06 aerial jump HUD", function(task, settings)
end, function(task, settings)
  local function goalComplete(goalCondition)
    if string.find(task.specialName, "At level 4") or string.find(task.specialName, "At level 3") then
  ...
  local function taskComplete()
    ...
    elseif task.specialName == "AGAIN Zap prompt" then
      feedbackSystem.menusMaster.primaryTextPrompt("ID:234456", nil, false, true, false, localPlayer.buttonLayout.zapUp)
    elseif task.specialName == "At level 4" then
    ...
    elseif task.specialName == "AGAIN At level 4" then
      feedbackSystem.updateTutorialPanel({tick1 = true})
      zapcontroller.setZapCameraLocks(0, {
        missile = true,
        low = true,
    ...
    elseif task.specialName == "At level 3" or task.specialName == "AGAIN At level 3" then
