local storedSteamID
feedbackSystem.registerHUD("Tanker on fire activity hud 2", function(task, settings)
end, function(task, settings)
  local waterLevelStart = 130
  ...
  if task.actor.ID == "Tanker4" then
    temperatureTable.slot = 2
    temperatureTable.title = "ID:184646"
  elseif task.actor.ID == "Tanker5" then
  ...
  local function goalComplete(conditionKey)
    if task.specialName == "Tanker payload" then
      if conditionKey >= 2 and conditionKey < 5 and (task.actor.ID == "Tanker4" or task.actor.ID == "Tanker5") then
        feedbackSystem.removeSlot(temperatureTable.slot)
        if task.actor.ID == "Tanker4" and not task.instance.taskObjectsByActorID.Tanker3 then
          temperatureTable.slot = 1
        elseif task.actor.ID == "Tanker5" and not task.instance.taskObjectsByActorID.Tanker3 and not task.instance.taskObjectsByActorID.Tanker4 then
          temperatureTable.slot = 1
        elseif task.actor.ID == "Tanker5" and (not task.instance.taskObjectsByActorID.Tanker3 or not task.instance.taskObjectsByActorID.Tanker4) then
          temperatureTable.slot = 2
        end
      end
    ...
    elseif task.specialName == "Third tanker" then
      if task.instance.taskObjectsByActorID.Tanker3 then
        task.instance.taskObjectsByActorID["Fire team member 3"].coreData.actor.spawn.relativeToVehicle.actor = "Tanker3"
      elseif task.instance.taskObjectsByActorID.Tanker4 then
        task.instance.taskObjectsByActorID["Fire team member 3"].coreData.actor.spawn.relativeToVehicle.actor = "Tanker4"
      elseif task.instance.taskObjectsByActorID.Tanker5 then
        task.instance.taskObjectsByActorID["Fire team member 3"].coreData.actor.spawn.relativeToVehicle.actor = "Tanker5"
      end
    end
  end
