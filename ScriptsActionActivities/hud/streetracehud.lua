feedbackSystem.registerHUD("Streetrace activity HUD", function(task, settings)
  ...
  local function update()
    if not gameStatus.simulationPaused and task.specialName == "racerTask" then
      local tempIndex = -1
      for k, v in next, instance.taskObjectsByActorID, nil do
      ...
      if tempIndex > index then
        ...
        index = tempIndex
        if index == #racerCheckpoints - 1 then
          feedbackSystem.menusMaster.primaryTextPromptParam({prompt = "ID:184379", priority = 1})
        end
        if index == math.ceil(#racerCheckpoints / 2) then
          feedbackSystem.menusMaster.primaryTextPromptParam({prompt = "ID:184380", priority = 1})
        end
      end
    end
  end
  local function goalComplete(conditionKey)
    if task.specialName == "ChaserTask" and (conditionKey == 1 or conditionKey == 2) then
      OneShotSound.Play("HUD_Gen_Positive", false)
      racersBar.numericValue = racersBar.numericValue + 1
      racersBar.value = math.ceil(racersBar.numericValue / totalRacers * 100)
      feedbackSystem.updateProgressBar(racersBar)
    elseif task.specialName == "Prompt ram - chaser" then
      ...
      for k, v in next, task.instance.taskObjectsByActorID, nil do
        if v.coreData.actor.team == "Race team" and 1 > v.coreData.agent.damage then
          local task = v.coreData.agent:getTaskObject()
          if task.namedTasks.racerTask and currentCheckpoint <= task.namedTasks.racerTask.networkVars.checkpoints - 1 then
            currentCheckpoint = task.namedTasks.racerTask.networkVars.checkpoints - 1
          end
        end
      end
      local racePercent = math.ceil(currentCheckpoint / #racerCheckpoints * 100)
      if conditionKey == 1 and racePercent <= 70 and racePercent >= 10 then
    ...
    elseif task.specialName == "Prompt headon - chaser" then
      ...
      local racePercent = math.ceil(currentCheckpoint / #racerCheckpoints * 100)
      if conditionKey == 1 and racePercent >= 70 and not feedbackSystem.menusMaster.primaryPromptActive then
        feedbackSystem.menusMaster.primaryTextPromptParam({prompt = "ID:243639", priority = 3})
      end
    end
  end
  local function taskComplete()
    if task.specialName == "Cop car destroyed" then
      feedbackSystem.menusMaster.primaryTextPromptParam({prompt = "ID:244202", priority = 1})
    end
  end
  local function cleanup()
    ...
    removeUserUpdateFunction(showPromptString)
  end
